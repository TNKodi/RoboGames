...
def moveMotor(error):
    ...
    if (speed+0.7*max_speed > max_speed):
        speed = 0.3*max_speed*error/speed
    else:speed=error
    left_motor.setVelocity(0.7*max_speed-speed)
    right_motor.setVelocity(0.7*max_speed+speed)
def pidController(kp,  kd, ki) :
    e = (dis_values[5]-dis_values[2])+100
    print('diff',e)
    global ePrevious
    global eIntegral
...
def isblock():
    ...
    if frontl[5]<1540 and frontr[5]<=1400:
        dir[0]= 0
    if right[5]>1980:
        dir[1]= 1
    if left[5]>1980:
